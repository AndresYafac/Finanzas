                  ...
                  return (
                    <article key={row.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 font-black text-white shadow-sm">
                            {(row.nombre?.[0] || 'U')}{(row.apellido?.[0] || '')}
                          </span>
                          <div>
                            <strong className="block text-sm font-black text-slate-900">{userName(row)}</strong>
                            <span className="text-xs font-semibold text-slate-500">{row.email_auth || row.email_contacto || '-'}</span>
                          </div>
                        </div>
                        <StatusBadge tone={status.tone}>{status.text}</StatusBadge>
                      </div>
                      <div className="mt-4 grid gap-2 text-sm text-slate-600">
                        <span><strong className="text-slate-900">Rol:</strong> {row.role || 'user'}</span>
                        <span><strong className="text-slate-900">Contacto:</strong> {row.telefono || row.email_contacto || '-'}</span>
                        <span><strong className="text-slate-900">Registro:</strong> {row.created_at ? new Date(row.created_at).toLocaleDateString('es-PE') : '-'}</span>
                      </div>
                      {row.id === user.id ? (
                        <span className="mt-4 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">Usuario actual</span>
                      ) : (
                        <div className="mt-4 flex flex-wrap gap-2">
                          <button className={neutralButton} type="button" onClick={() => openEdit(row)}><Pencil size={14} />Editar</button>
                          <button className={neutralButton} type="button" onClick={() => openPermissions(row)}><KeyRound size={14} />Permisos</button>
                          <button className={neutralButton} type="button" onClick={() => toggle(row)}>{row.activo ? 'Desactivar' : 'Activar'}</button>
                          <button className={dangerButton} type="button" onClick={() => remove(row)}><Trash2 size={14} />Eliminar</button>
                        </div>
                      )}
                    </article>
                  );
